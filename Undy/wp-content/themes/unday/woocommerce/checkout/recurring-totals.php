 ...
 <?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
			...
			<tr class="order-total recurring-total">
				<?php if ( $display_th ) : $display_th = false; ?>
					...
					<td rowspan="2" data-title="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></td> -->
				<?php else : ?>
					<?php  
					   $months = array(
					   	              'January' => 'januar',
					   ...
					   $month = date('F', strtotime($recurring_cart->next_payment_date));

 				      ?>
  					<td colspan="3"><strong><?php echo 'Medlemsskab hver 3 måned'; ?> </strong><div class="first-payment-date"><small>Første fornyelse bliver sendt afsted: <?php echo date('j. ', strtotime($recurring_cart->next_payment_date)); echo $months[$month]; echo date(' Y', strtotime($recurring_cart->next_payment_date)); 
