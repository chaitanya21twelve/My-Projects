	...
	class Pie_WCWL_Frontend_Init {
		...
		public function load_classes() {
			...
			if ( isset( $post->ID ) ) {
				...
				if ( is_product() || $product ) {
					...
					if ( $wc_product && array_key_exists( $wc_product->get_type(), WooCommerce_Waitlist_Plugin::$allowed_product_types ) ) {
						$this->load_class( $wc_product );
					}
				}
				...
				if ( function_exists( 'tribe_events_has_tickets' ) && tribe_is_event( $post->ID ) && 'yes' == get_option( 'woocommerce_waitlist_events' ) ) {
		...
		public function output_waitlist_elements( $atts ) {
			...
			if ( ! $product ) {
				...
				echo wcwl_get_waitlist_fields( $product_id );
			}
		}
