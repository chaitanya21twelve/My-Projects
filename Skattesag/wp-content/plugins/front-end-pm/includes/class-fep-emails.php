...
class Fep_Emails {
	...
	function send_email( $mgs, $prev_status ) {
		...
		$site_url = site_url(); 
		$redirect_url = $site_url."/beskeder";
		...
		if ( $participants && is_array( $participants ) ) {
			...
			$message  = __( 'Du har modtaget en ny besked fra skattesag.com', 'front-end-pm' ) . "\r\n";
			...
			//$message .= __( 'Please check your inbox to view full Message.', 'front-end-pm' ) . "\r\n";
			$message .= '<div cellspacing="0" cellpadding="0"><a href="'.$redirect_url.'" target="_blank" style="padding: 8px 12px; border: 1px solid #ED2939;border-radius: 2px;font-family: Helvetica, Arial, sans-serif;font-size: 14px; color: #ffffff;text-decoration: none;font-weight:bold;display: inline-block;background-color: #ED2939;">Log ind for at læse beskeden</a></div>';
			//$message .= __( 'Log ind for at læse beskeden', 'front-end-pm' ) . "\r\n";
