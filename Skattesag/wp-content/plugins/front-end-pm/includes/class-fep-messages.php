	...
	function get_column_content( $column ) {
		switch ( $column ) {
			...
			case 'author':
				if( 'threaded' === fep_get_message_view() ){
					?>
					<?php $participants = fep_get_participants( fep_get_the_id() );
					 //echo "<pre>"; print_r($participants); 
					$auth = $participants[0];
					  ?>
					<span class="fep-message-author"><?php echo fep_user_name( $auth ); ?></span>,  <span class="fep-message-date"><?php echo fep_get_the_date( 'mgs_last_reply_time' ); ?></span><?php
				} else {
			...
			case 'title':
				if ( ! fep_is_read( true ) ) {
					$span = '<span class="fep-unread-classp"><span class="fep-unread-class">' . __( 'Ulæst', 'front-end-pm' ) . '</span></span>';
					$class = ' fep-strong';
				} else {
					$span = '';
					$class = '';
				} 
				?><span><?php echo fep_get_the_title(); ?></a></span><?php echo $span; ?>
				<div class="fep-message-excerpt">
					<?php echo fep_get_the_content(); ?>
				</div>
				<span class="<?php echo $class; ?> readmore-btn"><a href="<?php echo fep_query_url( 'viewmessage', [
					'fep_id' => fep_get_the_id(),
					'feppage' => isset( $_GET['feppage'] ) ? $_GET['feppage'] : 1,
					'fep-filter' => isset( $_GET['fep-filter'] ) ? $_GET['fep-filter'] : '',
					] ); ?>"><?php echo "Læs mere"; ?></a></span>
				<?php
				break;
			default:
